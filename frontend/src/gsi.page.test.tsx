...
describe('GSI dedicated page', () => {
  ...
  it('starts with the Scale A/B chart after the introduction', () => {
    ...
    expect(resultAnalysis).toHaveTextContent('长沙有色冶金设计院根据工程经验')
    expect(resultAnalysis).not.toHaveTextContent('边界约定')
    expect(resultAnalysis).not.toHaveTextContent('等级名称')
  ...
  it('shows the Chinese Scale A/B chart by default and scores a clicked cell', () => {
    ...
    expect(screen.getByTestId('gsi-result-analysis').querySelector('.text-3xl')).toBeTruthy()
    expect(screen.getByTestId('gsi-result-analysis')).not.toHaveTextContent('计算代入')
  ...
  it('switches to quantitative inputs, formula, and the 4-row chart', () => {
    ...
    expect(calculator.className).toMatch(/(?:^|\s)thin-scroll(?:\s|$)/)
  ...
  it('plots the quantitative point and highlights the matching cell after typing Jcond89 and RQD', () => {
    ...
    expect(preview).toHaveTextContent('1.5')
    expect(preview).toHaveTextContent('RQD/2')
    expect(preview).toHaveTextContent(/刻度 A.*1\.5.*JCond89/)
    expect(preview).toHaveTextContent(/刻度 B.*RQD\/2/)

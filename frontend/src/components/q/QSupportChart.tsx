...
export default function QSupportChart({ result, darkMode = false, language = 'zh' }: { result: QResult | null; darkMode?: boolean; language?: 'zh' | 'en' }) {
  ...
  return (
    <svg data-testid="q-support-chart" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 870 438" role="img" aria-labelledby={`${id}-title ${id}-desc`} style={{ width: '100%', height: 'auto', fontFamily: 'Microsoft YaHei, Arial, sans-serif', background: darkMode ? '#172033' : '#ffffff' }}>
      ...
      {ticks.map((v) => <g key={v}><line x1={x(v)} x2={x(v)} y1={top} y2={bottom} stroke={grid} /><text x={x(v)} y={bottom + 22} textAnchor="middle" fontSize="12" fill={text}>{v}</text></g>)}
      {[0.1, 0.5, 1, 5, 10, 50, 100].map((v) => <g key={v}><line x1={left} x2={right} y1={y(v)} y2={y(v)} stroke={grid} /><text x={left - 10} y={y(v) + 4} textAnchor="end" fontSize="12" fill={text}>{v}</text></g>)}
      ...
      {inRange ? <line x1={x(result.q)} x2={x(result.q)} y1={top} y2={bottom} stroke={darkMode ? '#e2e8f0' : '#64748b'} strokeDasharray="4 4" /> : null}
      {showPoint ? <g data-testid="q-chart-point"><line x1={left} x2={x(result.q)} y1={y(result.equivalentDimension!)} y2={y(result.equivalentDimension!)} stroke={text} strokeDasharray="4 4" /><circle cx={x(result.q)} cy={y(result.equivalentDimension!)} r="6" fill="#2563eb" stroke="white" strokeWidth="2" /><title>{en ? 'Current point' : '当前点位'}: Q={formatQValue(result.q)}, De={formatQValue(result.equivalentDimension!)}</title></g> : null}

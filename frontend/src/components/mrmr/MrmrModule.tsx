...
import { buildBlankCase, buildBlankPoint, duplicatePoint, readCaseRecords, removeCase, sortCaseRecords, suggestCaseName, suggestPointName, formatTimestamp, upsertCase, writeCaseRecords } from '../../utils/rockmassCaseStore'
...
export default function MrmrModule({ darkMode, language, methodName, onBackToHome, handoff, onHandoffConsumed }: MrmrModuleProps) {
  ...
  useEffect(() => {
    ...
    const point = { ...buildBlankPoint('mrmr', handoff.source.pointName, handoff.form as unknown as Record<string, unknown>, MRMR_STANDARD.id, 3), note: handoff.source.note, oreType: handoff.source.oreType }
  ...
  const updatePoint = (pointId: string, patch: Partial<RockMassPointRecord>) => setDraft((current) => current ? ({ ...current, points: current.points.map((point) => point.id === pointId ? { ...point, ...patch, updatedAt: new Date().toISOString() } : point) }) : current)
  ...
  const createCase = () => { const name = newCaseName.trim() || (en ? `MRMR Classification ${formatTimestamp(new Date())}` : suggestCaseName('mrmr')); const record = buildBlankCase('mrmr', name, MRMR_STANDARD.id); const stored = persist(record, en ? `Case created: ${record.name}` : `已新建案例：${record.name}`); setDraft(stored); setStage('points'); setNewCaseName(en ? `MRMR Classification ${formatTimestamp(new Date())}` : suggestCaseName('mrmr')) }
  const importFiles = async (files: FileList | File[]) => { const imported: RockMassCaseRecord[] = []; const errors: string[] = []; for (const file of Array.from(files)) { const read = await readCaseFromFile(file, 'mrmr'); if ('record' in read) imported.push(read.record); else errors.push(en ? `${file.name}: ${englishErrorDetail(read.error, 'This MRMR case uses an unsupported old calculation version; create a new MRMR case.')}` : `${file.name}：${read.error}`) } if (imported.length > 0) { const next = [...imported, ...casesRef.current]; const write = writeCaseRecords(next); if (write.ok) { casesRef.current = next; setCases(next); setMessage(en ? `${imported.length} case(s) imported.` : `已导入 ${imported.length} 个案例。`) } } if (errors.length > 0) setMessage(errors.join(en ? '; ' : '；')) }

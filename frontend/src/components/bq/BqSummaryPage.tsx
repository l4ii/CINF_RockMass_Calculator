...
export default function BqSummaryPage({ darkMode, language, methodName, standard, caseRecord, rows, message, onBackToWorkspace, onBackToPoints, onOpenPoint, onOpenExport }: CustomSummaryPageProps) {
  ...
  const mathHeader = (math: string) => <span aria-hidden="true"><InlineMath math={math} /></span>
      ...
      <div className="thin-scroll min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6 lg:px-8">
        <div className="w-full space-y-5">
          ...
          <section className={`flex flex-wrap items-end gap-2 border-y py-3 ${border}`}>
            <label className="w-48 space-y-1"><span className={`text-xs font-medium ${muted}`}>{en ? 'Point / rock type' : '点位或矿岩类型'}</span><input className={`w-full ${input}`} value={query} onChange={(event) => setQuery(event.target.value)} /></label>
            ...
            <label className="w-28 space-y-1"><span className={`text-xs font-medium ${muted}`}>{en ? <><InlineMath math="R_c" /> min</> : <><InlineMath math="R_c" /> 最小值</>}</span><input aria-label={en ? 'Rc min' : 'Rc 最小值'} type="number" onWheel={preventNumberWheel} onKeyDown={preventNumberArrow} className={numberInputClass(`w-full ${input}`)} value={minRc} onChange={(event) => setMinRc(event.target.value)} /></label>
            <label className="w-28 space-y-1"><span className={`text-xs font-medium ${muted}`}>{en ? <><InlineMath math="R_c" /> max</> : <><InlineMath math="R_c" /> 最大值</>}</span><input aria-label={en ? 'Rc max' : 'Rc 最大值'} type="number" onWheel={preventNumberWheel} onKeyDown={preventNumberArrow} className={numberInputClass(`w-full ${input}`)} value={maxRc} onChange={(event) => setMaxRc(event.target.value)} /></label>
            <label className="w-28 space-y-1"><span className={`text-xs font-medium ${muted}`}>{en ? <><InlineMath math="K_v" /> min</> : <><InlineMath math="K_v" /> 最小值</>}</span><input aria-label={en ? 'Kv min' : 'Kv 最小值'} type="number" onWheel={preventNumberWheel} onKeyDown={preventNumberArrow} className={numberInputClass(`w-full ${input}`)} value={minKv} onChange={(event) => setMinKv(event.target.value)} /></label>
            <label className="w-28 space-y-1"><span className={`text-xs font-medium ${muted}`}>{en ? <><InlineMath math="K_v" /> max</> : <><InlineMath math="K_v" /> 最大值</>}</span><input aria-label={en ? 'Kv max' : 'Kv 最大值'} type="number" onWheel={preventNumberWheel} onKeyDown={preventNumberArrow} className={numberInputClass(`w-full ${input}`)} value={maxKv} onChange={(event) => setMaxKv(event.target.value)} /></label>

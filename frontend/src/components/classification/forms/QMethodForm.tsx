...
export default function QMethodForm({ form, onChange, darkMode, language }: MethodFormProps) {
  ...
  return (
    <>
      ...
      <MethodSection darkMode={darkMode} title={en ? 'Equivalent dimension and support' : '当量尺寸与支护需求判定'} description={state.supportMode === 'chart' ? (en ? 'Read permanent support from the Q–De chart; interpolate isolines and do not extrapolate dashed regions.' : '按 Q–De 支护图表读取永久支护，等值线内插，虚线区不外推。') : (en ? 'Screening against the empirical unsupported limit De = 2Q^0.4; verify against project conditions.' : '将当量尺寸与经验无支护极限 De＝2Q^0.4 比较，判定支护需求。')} source="De = span / ESR">
      <SelectField field="supportMode" label={en ? 'Support method' : '支护判定方法'} value={state.supportMode} options={[{ id: 'limit', label: '无支护极限', labelEn: 'Unsupported limit' }, { id: 'chart', label: '永久支护图', labelEn: 'Permanent support chart' }]} darkMode={darkMode} language={language} onChange={(supportMode) => patch({ supportMode: supportMode === 'chart' ? 'chart' : 'limit' })} />
